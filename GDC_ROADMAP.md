# GDC 추가 갱신 계획서
## Gopang Digital Currency — Development Roadmap v1.2

> **저장소:** `Openhash-Gopang/gdc`  
> **도메인:** `gdc.hondi.net`  
> **작성일:** 2026-06-03 (v1.1) / 2026-09-06 (v1.2 추가)  
> **작성자:** AI City Inc. (팀 주피터)  
> **현재 버전:** v1.0 (T1~T6 테스트 완료)

---

## § 0. v1.2 — 사이트 재설계 (2026-09-06)

`desktop.html` · `index.html` · `webapp.html` 세 파일을 원점에서 재설계했다.

| 파일 | 변경 내용 |
|------|----------|
| `desktop.html` | 이중 네비게이션(사이드바+상단바+배너 2개)을 단일 상단 네비 + 배너 1개로 정리. GDC 자체 문서 관행인 `§` 조항 번호를 화면에도 적용해 섹션 구조를 명세서 형식으로 통일. 신규 **§5 기술 기반** 절 추가 — 특허 출원 3건(민간 경쟁형 디지털 화폐 발행 시스템, FPGA 기반 AI 재무제표 검증 방법, OpenHash 적응형 계층 구조 시스템)의 기술 방향을 소개. AI 상담 모달·`_onGopangAuth` 콜백·`subsystem-auth.js` 등 기존 기능은 그대로 유지. |
| `webapp.html` | 색상 토큰만 desktop.html과 통일 (배경·테두리·텍스트 톤 정렬). 모든 함수명·엘리먼트 ID·API 호출은 변경하지 않음 — 실거래가 걸린 지갑 로직이라 시각 계층만 손댐. |
| `index.html` | 스플래시 배경·`theme-color`를 새 팔레트에 맞춰 조정. 라우팅 로직은 변경 없음. |

**§5 기술 기반 절 작성 원칙 — 반드시 지킬 것:**
- 특허 명세서의 처리 속도·정확도 등 구체 수치(예: 0.015ms, 133,333배, 99.4%)는 **검증되지 않은 출원 서류 기재값**이므로 사이트에 옮기지 않는다. 옮기면 과장광고·금융규제(자본시장법·유사수신행위법) 리스크가 생긴다.
- "특허 출원" ≠ "특허 등록" ≠ "실제 구현". 세 단계를 섞어 쓰지 않는다.
- 예금 이자·대출·투자·환전 등 인허가가 필요한 기능은 기존 §2·§3와 동일하게 "목표 — 컨소시엄 필요"로만 표기한다. 신규 절이라고 이 원칙을 느슨하게 적용하지 않는다.
