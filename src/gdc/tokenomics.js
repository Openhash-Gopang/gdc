/**
 * ✅ 활성화 (2026-07-18) — 법적 검토 판단: 이 모듈은 순수 계산 함수만
 * 제공한다(인플레이션율 공식, 소각 이력 기록, GEI 지수 계산) — 외부
 * 법정화폐를 수취하거나 GDC를 실제로 발행/이체하는 코드가 없다. 실제
 * 발행·소각 집행은 별도 시스템(예: gopang의 /api/mint, 아직 개발
 * 전용)이 담당하며, 그 발행주체 이슈는 js/gdc-pool.js LEGAL-HOLD 쪽에서
 * 계속 보류 중이다. 이 파일은 "정책 수치를 계산해서 보여주는" 용도로만
 * 활성화한다 — 이 파일의 함수 결과값을 근거로 실제 발행/소각을
 * 자동 집행하는 배선을 추가할 경우, 그 배선 지점은 다시 법적 검토
 * 대상이 된다는 점을 분명히 남겨둔다.
 */
/**
 * @file tokenomics.js
